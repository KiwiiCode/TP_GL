...
public class MainV3 {
	...
	public static void main(final String[] args) throws NotRightNumberException , java.io.IOException{
		...
		Observers journalist = new Journalist("Aya");
		Observers gambler = new Gambler("Marie");
		
		List<Observers> observers = new ArrayList<Observers>();
		observers.add(journalist);
		observers.add(gambler);
				...
				if (userInt == 0) {
					...
					switch (s) {
						...
						case "LAST":
							...
							break;
						default:
							System.out.println("Delault !");
					}
					master = new Master(competitors);
					master.setObservers(observers);
